...
def prop_decode_func(cls, prop, classes):
    ...
    if prop_is_list(prop):
        return f"DecodeList[{prop_go_type(prop, classes)}](value, path, {prop_ctx_name(cls, prop)}, {func})"

    return f"{func}(value, path, {prop_ctx_name(cls, prop)})"
...
def prop_encode_func(cls, prop, classes):
    ...
    if prop_is_list(prop):
        return f'EncodeList[{prop_go_type(prop, classes)}](self.{prop_name(prop)}.Get(), path.PushPath("{prop_name(prop)}"), {prop_ctx_name(cls, prop)}, {func})'

    return f'{func}(self.{prop_name(prop)}.Get(), path.PushPath("{prop_name(prop)}"), {prop_ctx_name(cls, prop)})'
...
class GoLangRender(BasicJinjaRender):
    ...
    def get_arguments(cls, parser):
        ...
        parser.add_argument("-p", "--package", help="Go Package Name", default="model")
